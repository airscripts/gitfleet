...
pub async fn run(cmd: CommentCmdCommand, app: &App) -> Result<(), GitfleetError> {
    ...
    match cmd {
        CommentCmdCommand::List { number, repo } => {
            ...
            let data = ops.list_change_comments(&repo_str, number).await?;
        ...
        CommentCmdCommand::Create { number, body, repo } => {
            let repo_str = crate::repo_util::resolve_repo(&repo)?;
            ...
            let result = ops.comment_on_change(&repo_str, number, &body).await?;
