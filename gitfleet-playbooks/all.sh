...
#   GITFLEET_PLAYBOOK_PARALLEL=1 bash gitfleet-playbooks/all.sh                   # run playbooks concurrently
#   GITFLEET_PLAYBOOK_SKIP="pipeline.sh,dependency.sh" bash gitfleet-playbooks/all.sh # skip playbooks
#   GITFLEET_PLAYBOOK_REPO=owner/repo GITFLEET_PLAYBOOK_ORG=orgname bash gitfleet-playbooks/all.sh  # override pointings
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/env.sh"

GITFLEET_PLAYBOOK_SKIP="${GITFLEET_PLAYBOOK_SKIP:-}"
GITFLEET_PLAYBOOK_PARALLEL="${GITFLEET_PLAYBOOK_PARALLEL:-0}"
...
should_skip() {
  ...
  for item in ${GITFLEET_PLAYBOOK_SKIP//,/ }; do
    item="${item%.sh}"
    if [ "$item" = "$name" ]; then
      return 0
    fi
...
run_playbook() {
  ...
  local output_file
  output_file=$(mktemp "$GITFLEET_PLAYBOOK_TMPDIR/gitfleet-playbook-${name}.XXXXXX")
  local exit_code=0
...
echo "[INFO] PROVIDER=$GITFLEET_PLAYBOOK_PROVIDER  GITFLEET_PLAYBOOK_REPO=$GITFLEET_PLAYBOOK_REPO  GITFLEET_PLAYBOOK_ORG=$GITFLEET_PLAYBOOK_ORG  GITFLEET_PLAYBOOK_TMPDIR=$GITFLEET_PLAYBOOK_TMPDIR"
echo ""

if [ "$GITFLEET_PLAYBOOK_PARALLEL" -eq 1 ]; then
  echo "[WARN] Parallel mode: running playbooks concurrently."
  echo "[WARN] Teardown order is not guaranteed in parallel mode."
  echo ""

  PARALLEL_DIR=$(mktemp -d "$GITFLEET_PLAYBOOK_TMPDIR/gitfleet-all.XXXXXX")
  PARALLEL_NAMES=()
  PARALLEL_PIDS=()
  PARALLEL_OUTPUTS=()
  ...
  for playbook in "${PLAYBOOKS[@]}"; do
    if should_skip "$playbook"; then
      RESULTS+=("$playbook: SKIPPED (in GITFLEET_PLAYBOOK_SKIP list)")
