...
teardown() {
  ...
  rm -f "$GITFLEET_PLAYBOOK_TMPDIR/snippet-$GITFLEET_PLAYBOOK_RESOURCE_SUFFIX.txt"
  print_summary
}
...
step "Snippet Create"
echo "gitfleet test snippet content" > "$GITFLEET_PLAYBOOK_TMPDIR/snippet-$GITFLEET_PLAYBOOK_RESOURCE_SUFFIX.txt"
output=$(gitfleet snippet create --description "gitfleet-test-snippet-$GITFLEET_PLAYBOOK_RESOURCE_SUFFIX" --public --file "$GITFLEET_PLAYBOOK_TMPDIR/snippet-$GITFLEET_PLAYBOOK_RESOURCE_SUFFIX.txt" --json 2>&1) || true
